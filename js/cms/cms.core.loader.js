...
(function(){
	...
	// Settings (loader)
	CMS.loader = {
		ScriptsBase: "/js/cms",        // Relative root to app JavaScript files

		NamespacePrefix: "cms",     // Used for loading external files from
		                            // separate directory

		ExtSubfolder: "externals",  // "Externals" folder name

		HostName: null,             // Host address, set on pageload

		SiteId: null,               // More params being set elsewhere
		UserEmail: null
	};

	CMS.loader.HostName = window.location.hostname;


	/**
	 * Loader for application resources.
	 * 
	 * Options for choosing resource location:
	 * - [http://*] absolute url
	 * - [/*] relative url to current host
	 *
	 * @param  {Array|String}  opts    Stuff to load
	 * @param  {Function}      [func]  Callback
	 */
	CMS.load = function(opts, func){
		var args = opts
		  , lder = this.loader
		  , hname = lder.HostName
		  , rootPath = lder.ScriptsBase
		  , nsPref = lder.NamespacePrefix
		  , i, path, arg, arr;
		  
		rootPath = "http://" + hname + rootPath;
		...
		for (i = 0; i < args.length; i++) {
			...
			if ("http" === arg.substr(0,4)) {// absolute url
				path = arg;
			} else if ("/" === arg.substr(0,1)) {// relative url
				path = arg;
			} else {
				if (arg.indexOf("/") === -1) {// subfolder not specified
					if (arg.substr(0, nsPref.length) !== nsPref) {
						arg = lder.ExtSubfolder + "/" + arg;
					}
				}
		...
		if (typeof func === "function") arr.push(func);
		return head.js.apply(head, arr);// call head.js loader
	};


	/**
	 * Head JS 0.96 (c) Tero Piirainen (tipiirai) http://bit.ly/mit-license
	 * @see http://headjs.com
	 */
